...
// ---- Cleanup invalid trades on load ----
window.addEventListener("load", () => {
  trades = trades.filter(t => t.symbol && !isNaN(t.entry) && !isNaN(t.exit));
  save();
  renderAll();
});
...
document.getElementById('exportPdfBtn').addEventListener('click', async () => {
  ...
  const report = document.createElement('div'); report.style.padding = '18px'; report.style.background = 'white'; report.style.color = '#111';
  report.innerHTML = `<h2>Backtesting Report</h2><b>Total Trades:</b> ${trades.length} &nbsp; <b>Net P/L:</b> ${trades.reduce((a, b) => a + (Number(b.pl) || 0), 0).toFixed(2)}`;
  document.body.appendChild(report);
  await new Promise(r => setTimeout(r, 300));
  const canv = await html2canvas(report, { useCORS: true, scale: 1 });
...
// ✅ Save function - sirf localStorage me trades dump karega
function save() {
  try {
    localStorage.setItem('tj_ftmo_v1', JSON.stringify(trades));
  } catch (e) {
